...
render_gene_report <- function(input, type, output_file) {
  ...
  p1 <- make_celldeps(celldeps_data = achilles, expression_data, gene_symbol, mean = mean_virtual_achilles)
  p2 <- make_cellbins(cellbins_data = achilles, expression_data, gene_symbol)
  p3 <- make_lineage(celldeps_data = achilles, expression_data, gene_symbol)
  p4 <- make_sublineage(celldeps_data = achilles, expression_data, gene_symbol)
